...
unsigned lodepng_miniz_decoder(unsigned char** out, size_t* outsize,
	...
	printf("DECODE out: %p (%ld), in: %p (%ld)\n", *out, *outsize, in, insize);
	...
	*out = (unsigned char *)malloc(*outsize);
	...
	int res = mz_uncompress(*out, outsize, in, insize);

	//printf("DECODE out: %p (%ld), in: %p (%ld), result: %d\n", *out, *outsize, in, insize, res);
...
unsigned lodepng_miniz_encoder(unsigned char** out, size_t* outsize,
	...
	*out = (unsigned char *)malloc(*outsize);
	...
	int res = mz_compress2(*out, outsize, in, insize, MZ_DEFAULT_LEVEL + 2);

	//printf("ENCODE out: %p (%ld), in: %p (%ld), result: %d\n", *out, *outsize, in, insize, res);
